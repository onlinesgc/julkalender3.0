...
export class SendDayCommand implements Command {
    ...
    async sendDay(channel: GuildTextBasedChannel, dayNum: number, clientId: string) {
        ...
        await channel.send({ content: clientData?.adventPingRole ? `<@&${clientData.adventPingRole}>` : "", components: [row] });
    }
}